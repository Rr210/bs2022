 ...
 * @LastEditTime: 2022-05-03 16:36:00
 * @LastEditors: harry
-->
<template>
  <van-nav-bar title="Pest identification">
    ...
    <template #left>
      <!-- 展示登录的状态框 -->
      <van-popover v-model:show="showPopover" placement="right-start" :actions="actions" @select="onSelect">
        <template #reference>
          <van-image v-if="isloginstate" class="pic_artar" width="1rem" height="1rem" fit="contain"
  ...
  <van-tabbar v-model="active" route>
    ...
    <van-tabbar-item icon="clock-o" replace :to="{ name: 'history' }" :badge="dotnum">历史</van-tabbar-item>
    <van-tabbar-item icon="setting-o" replace :to="{ name: 'mine' }" dot>设置</van-tabbar-item>
  </van-tabbar>
</template>
